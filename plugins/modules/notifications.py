...
from ..module_utils.notifications_api import notificationApi
__metaclass__ = type


DOCUMENTATION = r'''
module: notifications
short_description: 'This module allows you to work with networker notifications'
description: 'Use this module to create, delete, modify notifications on networker server'
version_added: '2.0.0'
options:
  state:
    type: str
    ...
    required: true
    description: 'Specify the action you want to take. e.g for creating notification, use create'
  name: 
    type: str
    description: 'Name of the notification resource'
  comment:
    type: str
    description: 'Any user defined description of this notification'
  action:
    type: str
  ...
  additionalEmailRecipient:
    type: str
    description: 'specifies one or more email addresses of the intended recipients to be posted on occurrence of the events defined in the notification. Use a comma to separate multiple email addresses.'
  enabled:
    type: bool
...
- name: Get all notifications
  dellemc.networker.notifications:
    state: get
...
def main():
    ...
    resp_msg['responses'] = []
    resp_msg['msg'] = []
    api_initialize = notificationApi(auth=auth, url=url)

    if state == 'get':
        if module.params['name'] is not None:
            if len(module.params['name']) > 0:
                notification_name = module.params['name']
                response = api_initialize.get_notification(notification_id=notification_name)
                resp_msg['responses'].append(response)
        else:
            response = api_initialize.get_notifications(query_params=module.params['query_params'], field_params=module.params['field_params'])
            resp_msg['responses'].append(response)
    elif state == 'create':
        notification_name = module.params['name']
        body = params
        del body['state']
        response = api_initialize.post_notification(body=body)
        resp_msg['responses'].append(response)
        msg = 'notification %s Created Successfully' % notification_name
        resp_msg['msg'].append(msg)
    elif state == 'modify':
        notification_name = module.params['name']
        body = params
        del body['state']
        response = api_initialize.put_notification(body=body, notification_id=notification_name)
        resp_msg['responses'].append(response)
        msg = 'notification %s modified Successfully' % notification_name
        resp_msg['msg'].append(msg)

    elif state == 'delete':
        notification_name = module.params['name']
        response = api_initialize.delete_notification(notification_id=notification_name)
        resp_msg['responses'].append(response)
        msg = 'notification %s deleted Successfully' % notification_name
        resp_msg['msg'].append(msg)
